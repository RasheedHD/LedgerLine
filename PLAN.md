...
| **D48** | **CI is not executing.** Since 2026-08-26 pushes create no workflow run at all, and a manually dispatched run sat queued without starting a job. The workflow file is valid, LF-only, identical on the remote, Actions reports operational and the workflow active. `actions/checkout` and `setup-go` were bumped v4/v5 to v7 because the Node 20 grace period ended, and `workflow_dispatch` was added so a run can be started by hand. **The v7 bump is unverified** - no run has executed since `810dd2c` on 2026-08-19. Check the Actions tab first thing on the next machine | ADR-0017 | 8 |
